...
- Size: `360 × 220`
- Corner radius: `34`
- Main status expression: bottom status strip plus center orb
  - left strip label: dynamic global display title
    - `idle` -> `IDLE`
  ...
  - right strip label: compact `M...` readout
  - center orb: embedded in the gold ring and visually above the strip
  - no horizontal red/pink glow bleed outside the center orb
- Mascot: left side, cute lucky-cat style
- Left front paw: one continuous shell-to-paw silhouette with no detached seam
