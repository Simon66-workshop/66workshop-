...
struct LuckyCatCompactShell<Content: View>: View {
    ...
    private var sidePawAndBell: some View {
        HStack {
            Spacer()
            VStack(spacing: 0) {
                LuckyCatSidePaw()
                ...
                VStack(spacing: 0) {
                    Capsule(style: .continuous)
                        ...
                        .offset(x: 22, y: 1)
                    Circle()
                        .stroke(LuckyCatTokens.Palette.goldDeep.opacity(0.9), lineWidth: 2.4)
                ...
                .rotationEffect(.degrees(bellSwing ? 6 : -6), anchor: .top)
                .animation(.easeInOut(duration: 1.4).repeatForever(autoreverses: true), value: bellSwing)
                .onAppear {
                    bellSwing = true
                }
                Spacer()
