...
In M3.2c+, this watcher is signal-only for UI lifecycle:

- it appends normalized `heartbeat` / `release` signals into
  `normalized_signals.jsonl`
- it updates `thread_bindings/<CODEX_THREAD_ID>.json`
- it does not need to mutate task JSON just to keep LuckyCat blue
- the State Projector uses watcher signals plus thread-binding freshness to
  decide whether the current-thread task still projects as `active_execution`
...
not updated within that lease, the watcher marks the binding `released` and
emits a release signal, so an idle Codex session does not remain blue
indefinitely.
...
- managed running/pending keeps the lamp blue,
- a fresh high-confidence appserver runtime candidate can also keep the lamp blue,
- raw `process_observer` and weak/global-only private probe records are
  diagnostic-only and do not keep the lamp blue by themselves,
...
LuckyCat read model. The projector reads `normalized_signals.jsonl` as the
primary UI signal bus, then uses task files, `state.json`, turn bindings,
observations, hook bridge health, and UI client diagnostics as enrich/compat
inputs before atomically writing a single UI-safe snapshot.
...
Projector diagnostics include hook bridge status, signal bus status, latest
signal age, active turn binding count, latest active turn age, latest observed
age, mismatch warnings, state dir, app bundle path, build id, and projector
reasons.

Current-thread lifecycle diagnostics may also include:
...
`normalized_signals.jsonl` is an append-only sanitized local signal bus written
by hook / observer / probe / explicit producers. It must not contain prompt,
response, auth, or raw log body content.

The signal bus is locally bounded. Writers may compact it under the same lock
...
3. Else if a fresh high-confidence appserver runtime candidate with active-like
   evidence is active, the lamp is blue.
...
## M3.3 UI Read Model Additions

`ui_state.json` remains a read model, not a task writer protocol. M3.3 adds
diagnostic and arbitration fields so the UI can explain why the lamp is red,
blue, pending, green, or idle.

Top-level writer identity fields:
...
Each runtime candidate contains:
...
`process_observer` and global-only private probe signals may appear in
`runtime_candidates` or observation counts, but they must not independently
