...
rg -q "@main" "$SCAFFOLD" || fail "WidgetBundle entrypoint is missing"
rg -q "WidgetBundle" "$SCAFFOLD" || fail "WidgetBundle scaffold is missing"
rg -q "case \\.systemMedium" "$SCAFFOLD" || fail "medium widget layout is missing"
rg -q "supportedFamilies\\(\\[\\.systemSmall, \\.systemMedium\\]\\)" "$SCAFFOLD" || fail "small/medium widget families are not declared"
