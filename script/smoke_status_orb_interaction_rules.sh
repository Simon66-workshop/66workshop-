...
EVENT_REPLAY="$ROOT_DIR/script/smoke_tasklight_interaction_event_replay.sh"
...
rg -q "TaskLightInteractionStateMachine" "$PANEL" "$MACHINE" || fail "interaction decisions must come from the shared state machine"
...
rg -q "cross_surface_double_tap_opened_diagnostics" "$PANEL" "$BUILD" "$EVENT_REPLAY" || fail "runtime replay must prove cross-surface double tap"
